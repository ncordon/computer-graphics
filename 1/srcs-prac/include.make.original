...
## *********************************************************************
## librerías para el enlazador (puede depender del S.O.)
...
ifeq ($(uname),Darwin)
    ...
    os          := LINUX
    lib_gl_std  := -lGL
    lib_gl_nv   := /usr/lib/nvidia-340-updates/libGL.so
    ifeq ($(wildcard $(lib_gl_nv)),$(lib_gl_nv))
   	    lib_gl  := $(lib_gl_nv)
    else
   	    lib_gl  := $(lib_gl_std)   
    endif
...
## flags compilador:
os_flag  := -D$(os)
## c_flags  := -I$(include_dir) -I$(extra_inc_dir) $(os_flag) $(opt_dbg_flag) $(exit_first) $(warn_all) -std=c++11
